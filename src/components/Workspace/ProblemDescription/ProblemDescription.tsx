...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem, _solved }) => {
  ...
  const [updatingStar, setUpdatingStar] = useState<boolean>(false);
  ...
  const handleLike = async () => {
    ...
    await runTransaction(firestore, async (transaction) => {
      const {problemDoc, problemRef, userDoc, userRef} = await returnUserDataAndProblemData(transaction);
        ...
        <div className='px-5'>
          ...
          <div className='w-full'>
            ...
            >
              ...
              {!updating && <AiFillLike className={`${liked ? 'text-dark-blue-s' : ''}`}  />}
            ...
            >
              {!updating && <AiFillDislike className={`${disliked ? 'text-dark-blue-s' : ''}`} />}
              {updating && <AiOutlineLoading3Quarters className="animate-spin" />}
            ...
            <div className='cursor-pointer hover:bg-dark-fill-3  rounded p-[3px]  ml-4 text-xl transition-colors duration-200 text-green-s text-dark-gray-6 '
              onClick={handleStar}
            >
              {starred && !updatingStar && <TiStar className='text-dark-yellow' />}
              {!starred && !updatingStar && <TiStarOutline />}
              {updatingStar && <AiOutlineLoading3Quarters className="animate-spin" />}
            </div>
          </div>
            )}
