...
const Topbar: React.FC<TopbarProps> = ({ problemPage }) => {
  ...
  const handleProblemChange = (isForward: boolean) => {
    ...
    const nextProblem = order + direction;
    const nextProblemKey = Object.keys(problems).find(key => (problems[key] as Problem).order === nextProblem);

    if(isForward && !nextProblemKey) {
      const firsProblemKey = Object.keys(problems).find(key => (problems[key] as Problem).order === 1);
    ...
    <nav className='relative flex h-[50px] w-full shrink-0 items-center px-5 bg-dark-layer-1 text-dark-gray-7'>
      <div className={`flex w-full items-center justify-between ${problemPage ? 'max-w-[1200px] mx-auto' : ''}`}>
        <Link href='/' className='h-[22px] flex-1'>
          <Image src='/logo-full.png' alt='LeetClone' width={100} height={100} />
        </Link>
